...
class Network:
    def __init__(self, frame_height, frame_width, exp_name):
        ...
        self.train_writer = tf.summary.FileWriter("./logs/network_modif_2/train/%s" % exp_name, self.session.graph)
        self.test_writer = tf.summary.FileWriter("./logs/network_modif_2/test/%s" % exp_name)

    def construct(self):
        print("================")
        with self.session.graph.as_default():
            self.input_frames = tf.placeholder(tf.float32, [None, self.frame_height, self.frame_width, 3, 2])
            print("\tself.input_frames: " + str(self.input_frames.get_shape()))            
            self.gold_output_frames = tf.placeholder(tf.int32, [None, self.frame_height, self.frame_width, 3, 1])
