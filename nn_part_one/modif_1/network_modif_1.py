...
class Network:
    def __init__(self, frame_height, frame_width, exp_name):
        ...
        graph = tf.Graph()
        self.session = tf.Session(graph=graph, config=tf.ConfigProto(intra_op_parallelism_threads=4))       
    
        self.train_writer = tf.summary.FileWriter("./logs/network_modif_1/train/%s" % exp_name, self.session.graph)
    ...
    def construct(self):        
        with self.session.graph.as_default():
            ...
            self.__print_tensor_shape(self.input_frames, "\tinput_frames")

            self.gold_output_frames = tf.placeholder(tf.float32, [None, self.frame_height, self.frame_width, 1])
            self.__print_tensor_shape(self.gold_output_frames, "\tgold_output_frames")

            conv_layer1 = tf.layers.conv2d(self.gold_output_frames, 48, [1, 3], 1, padding="SAME")
            self.__print_tensor_shape(conv_layer1, "\tconv_layer1")

            conv_layer2 = tf.layers.conv2d(conv_layer1, 48, [3, 1], 1, padding="SAME")
            ...
            self.loss = tf.losses.sigmoid_cross_entropy(self.gold_output_frames, output_layer)            
